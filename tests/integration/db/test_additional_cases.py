...
async def test_auth_login_blocks_on_too_many(monkeypatch):
    ...
    cache = FakeCache()
    monkeypatch.setattr("app.services.user.CacheService", lambda *a, **k: cache)

    dummy_user = types.SimpleNamespace(_antifraud_ttl_seconds=UserService._antifraud_ttl_seconds)

    async def antifraud(request):
        return await UserService.antifraud(dummy_user, request)

    service = AuthBusinessService()
    service.user_service = types.SimpleNamespace(
        antifraud=antifraud,
        login=lambda data: (_ for _ in ()).throw(AssertionError("no call")),
    )
...
async def test_auth_refresh_invalid_token(monkeypatch):
    service = AuthBusinessService()
    async def get_user_from_refresh_token(request):
        return await UserService.get_user_from_refresh_token(object(), request)

    service.user_service = types.SimpleNamespace(get_user_from_refresh_token=get_user_from_refresh_token)
    monkeypatch.setattr("app.services.user.verify_token", lambda token: (_ for _ in ()).throw(JWTError()))

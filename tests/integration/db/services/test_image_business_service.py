...
from tests.factories import create_file, create_image, create_location, create_room, create_user


def test__build_key_uses_scope_owner_ext(monkeypatch):
    monkeypatch.setattr(
        "app.services.file.uuid.uuid4",
        lambda: types.SimpleNamespace(hex="abc123"),
    )

    key = FileService.build_key("rooms", 42, "png")
...
@pytest.mark.asyncio
async def test__upload_location_image_rejects_zero_size(db_session, faker, monkeypatch):
    user = await create_user(db_session, faker)
    location = await create_location(db_session, faker)
    await db_session.flush()
    token = SAccessToken(sub=str(user.id), admin=True)
    ...
    payload = SImageUploadIn(mime="image/png", ext="png", size=0, original_name="loc.png")
    with pytest.raises(HTTPException) as exc:
        await ImageBusinessService(token_data=token).upload_location_image(location.id, payload)
    assert exc.value.status_code == 422
...
@pytest.mark.asyncio
async def test__upload_location_image_rejects_null_byte_name(db_session, faker, monkeypatch):
    user = await create_user(db_session, faker)
    location = await create_location(db_session, faker)
    await db_session.flush()
    token = SAccessToken(sub=str(user.id), admin=True)
...
@pytest.mark.asyncio
async def test__upload_location_image_allows_multiple_images(db_session, faker, monkeypatch):
    user = await create_user(db_session, faker)
    location = await create_location(db_session, faker)
    await db_session.flush()
    token = SAccessToken(sub=str(user.id), admin=True)
    ...
    result = await db_session.execute(select(Image).where(Image.location_id == location.id))
